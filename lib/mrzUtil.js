...
const createQR = (data) => {
  ...
  // 발급입 추가
  let body = `1A${documentCode}:${issuingState}:${documentNumber}:${nationality}:${expirationDate}:${personalNumber}:${birthDate}:${sex}:${firstName}:${lastName}:${getFormatDate(
    new Date()
  )}`;

  console.log('[QRCODE STRING]: ' + body);
  return body;
};
...
const createBAR = ({ nationality, documentNumber }) => {
  ...
  // 발급일 추가
  const body =
    '1' + '1' + getFormatDate(new Date()) + nationality + documentNumber;
  console.log('[BARCODE STRING]:' + body);
  return body;
};
